...
if(empty($error)){
	$Tm = '1';
$TmTT = '0';
$regs = $flash->prepare("INSERT INTO `user` (username,email,password,firstname,lastname,phone,bankname,accname,accno,location,joined,token,extra,referral,tomerge,`right`) 
	VALUES (:uname,:email,:pass,:fname,:lname,:phone,:bname,:aname,:ano,:loc,:joined,:token,:extra,:ref,:rg,:rvv)");
$regs->bindParam(':uname',$uname);
$regs->bindParam(':email',$uemail);
$regs->bindParam(':rg',$Tm);
$regs->bindParam(':rvv',$TmTT);
